...
export function createPaymentScene() {
  ...
  function mount(ctx: { gradientHost: HTMLDivElement; contentHost: HTMLDivElement }): void {
    ...
    heroImg.src = '/pic/pic_payment.png';
    heroImg.alt = '';
    ...
    `;
    quote.innerHTML =
      '如同康定斯基用最基础的点、线、面构建出一种绝对的平衡与流动感，<br/>我统一视觉、优化流程，构建出高效流动的数字化支付秩序。';
    ...
    pageTitle.textContent = '北美支付应用优化升级';
    ...
    void (async () => {
      const raw = await fetch(PAYMENT_MD_URL).then((r) => r.text());
      article.replaceChildren();
      buildArticleNodes(article, raw);
    })();

    const tailImg = document.createElement('img');
    tailImg.src = '/pic/pic_payment2.png';
    tailImg.alt = '';
