...
const TICKET_MD_URL = '/content/ticket.md';

export function createTicketScene() {
  ...
  function mount(ctx: { gradientHost: HTMLDivElement; contentHost: HTMLDivElement }): void {
    ...
    heroImg.src = '/pic/pic_ticket.png';
    heroImg.alt = '';
    ...
    `;
    quote.innerHTML =
      '工单系统不是静态的存档，它是驱动企业内部流程“走起来”的动力引擎。<br/>通过工单系统的秩序，让企业的每一个协作节点都迈出高效的步伐。';
    ...
    void (async () => {
      const raw = await fetch(TICKET_MD_URL).then((r) => r.text());
      article.replaceChildren();
      buildArticleNodes(article, raw);
    })();

    const tailImg = document.createElement('img');
    tailImg.src = '/pic/pic_ticket2.png';
    tailImg.alt = '';
