...
export function createMPoCScene() {
  ...
  function mount(ctx: { gradientHost: HTMLDivElement; contentHost: HTMLDivElement }): void {
    ...
    `;
    quote.innerHTML =
      '如同《思想者》在静默中审视世界，<br/>我将复杂的安全数据，提炼为笃定的决策智慧。';
    ...
    pageTitle.textContent = 'MPoC 安全监控与认证平台设计';
    ...
    void (async () => {
      const raw = await fetch(MPoC_MD_URL).then((r) => r.text());
      article.replaceChildren();
      buildArticleNodes(article, raw);
    })();

    const tailImg1 = document.createElement('img');
    tailImg1.src = '/pic/pic_MPoC2.jpg';
    tailImg1.alt = '';
    tailImg1.style.cssText = `
      display: block;
      width: 100%;
      height: auto;
      margin-top: 28px;
      ...
      user-select: none;
    `;
    scrollInner.appendChild(tailImg1);

    const tailImg2 = document.createElement('img');
    tailImg2.src = '/pic/pic_MPoC3.jpg';
    tailImg2.alt = '';
    tailImg2.style.cssText = `
      display: block;
      width: 100%;
      height: auto;
      margin-top: 8px;
      ...
      user-select: none;
    `;
    scrollInner.appendChild(tailImg2);
